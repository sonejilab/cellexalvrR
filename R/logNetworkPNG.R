...
#' logNetworkPNG is a VR helper funtion that stores one network plot into the log document.
#' @name logNetworkPNG
#' @docType methods
...
#' @export
setGeneric("logNetworkPNG", function(cellexalObj, genes = NULL, png, grouping, ...) {
    standardGeneric("logNetworkPNG")
})


#' @rdname logNetworkPNG
setMethod("logNetworkPNG", signature = c("cellexalvrR"), 
    ...
    if (!file.exists(png)) {
        stop(paste("logNetworkPNG the network png file can not be found!", "png"))
    }
    ...
    content = paste(
            paste("##", "Network from Saved Selection", basename(gInfo@selectionFile)), 
            paste("This selection is available in the R object as group", gInfo@gname), "", 
...
#' @rdname logNetworkPNG
setMethod("logNetworkPNG", signature = c("character"), definition = function(cellexalObj,
    genes = NULL, png, grouping, ...) {
    cellexalObj <- loadObject(cellexalObj)
    logNetworkPNG(cellexalObj, genes, png, grouping, ...)  #function definition in file 'logNetworkPNG.R'
})
